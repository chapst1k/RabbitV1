...
const api = {
    ...
    createBreeding: (breeding) => {
        const { maleName, femaleName, maleSpecies, femaleSpecies, ...pureData } = breeding;
        return fetch(`${API_BASE}/breedings`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    createHatching: (hatching) => {
        const { hatchRate, ...pureData } = hatching;
        return fetch(`${API_BASE}/hatchings`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(pureData)
        }).then(res => res.json());
